...
const BottomAlignment = () => {
  const onClick = () => {
    ...
    list.map((item:any)=>{
       let y
       if(item._private.autoHeight!=undefined){
        y = maxY - item._private.bodyBounds.h/2
       }else{
        y = maxY - item._private.bodyBounds.h/2
       }
       item.position('y',y)
  ...
  return (
    <Tooltip placement="bottom" title={'底部对齐'}>
      <Button className='export-button' icon={<IconFont type="icon-xiaduiqi"/>} onClick={onClick} size='large' />
    </Tooltip>
  );
};
